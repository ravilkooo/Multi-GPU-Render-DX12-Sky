#ifndef RENDER_SKY_MARCHING_HLSL
#define RENDER_SKY_MARCHING_HLSL

#include "AtmosphereCommon.hlsl"
#include "ComputeSkyCommon.hlsl"
...
SingleScatteringResult IntegrateScatteredLuminance(
	...
	in bool MieRayPhase, in uint2 currResolution, in float tMaxMax = 9000000.0f)
{
    const bool debugEnabled = all(uint2(pixPos.xx) == gMouseLastDownPos.xx) && uint(pixPos.y) % 10 == 0 && DepthBufferValue != -1.0f;
    SingleScatteringResult result = (SingleScatteringResult) 0;

    float3 ClipSpace = float3((pixPos / float2(currResolution)) * float2(2.0, -2.0) - float2(1.0, -1.0), 1.0);
...
#undef SQRTSAMPLECOUNT
    ...
    MultiScatOut[ThreadId.xy] = float4(MultipleScatteringFactor * L, 1.0f);
}
...
[numthreads(32, 32, 1)]
void ComputeTransmittanceLutCS(uint3 dispatchThreadId : SV_DispatchThreadID)
{
	// TO-DO: Check that viewHeight and viewZenithCosAngle values,
	// that raySphereIntersectNearest(...) return -1.0 and we don't go inside cycle
...
#endif // RENDER_SKY_MARCHING_HLSL
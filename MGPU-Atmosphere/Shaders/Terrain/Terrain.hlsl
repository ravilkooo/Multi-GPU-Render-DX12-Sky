...
#if 0
	const float height = terrainHeightMap.SampleLevel(gsamLinearClamp, Uvs, 0).r;
#else
    const float offset = 0.0008;
    float HeightAccum = terrainHeightMap.SampleLevel(gsamLinearClamp, Uvs + float2(0.0f, 0.0f), 0).r;
    HeightAccum += terrainHeightMap.SampleLevel(gsamLinearClamp, Uvs + float2(offset, 0.0f), 0).r;
    HeightAccum += terrainHeightMap.SampleLevel(gsamLinearClamp, Uvs + float2(-offset, 0.0f), 0).r;
...
VertexOut TerrainVS(VertexIn vin, uint vertexId : SV_VertexID, uint instanceId : SV_InstanceID)
{
    ...
    output.TexC = Uvs;
    output.PosView = mul(WorldPos, worldBuffer.ViewProj);
    ...
    output.SsaoPosH = mul(WorldPos, worldBuffer.ViewProjTex);
